...
def Plot_indominating_cases_for_M1L1(df,dict_name):
    #select the df
    ...
    plt.plot(ff_LR2[ff_LR2>=0], S_bias_LR2[ff_LR2>=0], label="LR2", linestyle="-.",color='blue')  
    plt.plot(ff_Matern[ff_Matern>=0], S_bias_Matern[ff_Matern>=0], label="Matern", linestyle="-.",color='green')   
    plt.plot(ff_M1L1[ff_M1L1>=0], S_bias_M1L1[ff_M1L1>=0], label="M1L1",color='black') 
    plt.title(dict_name)
    plt.xlim(0.5,220)
    plt.ylabel("PSD (K²/cpd)")
    plt.xlabel("Frequency (cpd)")
...
def Plot_pair_plots(df, columns, colors):
    import seaborn as sns
    import matplotlib.pyplot as plt
...
def Plot_depth_box_plot(locations,lat_list,data_df,bin_size,
    ...
    for parameter_order,parameter_name in enumerate(parameter_names):
        for i, loc in enumerate(locations):
            ...
            axes[parameter_order].set_xlabel('{} ({})'.format(parameter_name_subscript,parameter_units[parameter_order]),fontsize = 30)
    ...
    labels = locations   
    fig.legend(handles, labels, bbox_to_anchor=(0.48, 0.65),fontsize=20)  
    ...
    if save:
        ...
        plt.savefig('{} depth profile of parameter.png'.format(locations), dpi=600, transparent=True, bbox_inches='tight')
...
def Plot_seasonal_variability(df,parameter_name,parameter_units,lat_list,
    ...
    for parameter_order,parameter_name in enumerate(parameter_name):
        ...
        axes[parameter_order*2].set_ylabel('{} ({})'.format(parameter_name_subscript ,parameter_units[parameter_order]),fontsize = 25)
    ...
    if save:
        ...
        plt.savefig('Seasonal_variability_of_parameter_at_max_eta_and_whole_depth.png', dpi=600, transparent=True, bbox_inches='tight')
    #plt.show() 
...
def Plot_obs_subplots(time_list, A_list, Mean_params_list, F_GM, P_GM):
    ...
    ax1.plot(F_obs, Peaks, label='Haramonic Analysis')
    ax1.plot(F_GM, P_GM, label='Garret-Munk', linewidth=2.5)
    ax1.axvline(x=O1_freq, color='g', linestyle='--', alpha=0.5)
    ax1.axvline(x=K1_freq, color='g', linestyle='--', alpha=0.5)
    ax1.axvline(x=M2_freq, color='b', linestyle='--', alpha=0.5)
    ax1.axvline(x=S2_freq, color='b', linestyle='--', alpha=0.5)
    ax1.axvline(x=f_coriolis_cpd, color='black', linestyle='--', alpha=0.5)
    ax1.text(f_coriolis_cpd - 0.05, 0.9, 'f', color='black')
    ax1.text(O1_freq - 0.25, 0.9, 'O1 K1', color='g')
    ax1.text(S2_freq - 0.55, 0.9, 'M2 S2', color='b')
    ax1.set_xscale("log")
    ax1.set_yscale("log")
    ...
    ax2.plot(F_obs, Peaks, label='Haramonic Analysis')
    # ax2.plot(F_GM, P_GM, label='Garret-Munk', linewidth=2.5)
    ax2.axvline(x=O1_freq, color='g', linestyle='--', alpha=0.5)
    ax2.axvline(x=K1_freq, color='g', linestyle='--', alpha=0.5)
    ax2.axvline(x=M2_freq, color='b', linestyle='--', alpha=0.5)
    ax2.axvline(x=S2_freq, color='b', linestyle='--', alpha=0.5)
    # ax2.axvline(x=f_coriolis_cpd, color='black', linestyle='--', alpha=0.5)
    # ax2.text(f_coriolis_cpd - 0.05, 0.9, 'f', color='black')
    ax2.text(O1_freq-0.05, 0.9, 'O1  K1', color='g')
    ax2.text(S2_freq-0.1, 0.9, 'M2  S2', color='b')
